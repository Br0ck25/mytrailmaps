...
useEffect(() => {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      {/* Sidebar */}
      <div className={`bg-gray-100 w-64 p-4 space-y-4 border-r border-gray-300 transition-all duration-300 ease-in-out ${menuOpen ? "block" : "hidden md:block"}`}>
        <h1 className="text-2xl font-bold text-green-700">MyTrailMaps</h1>
        <nav className="space-y-2">
          <button onClick={() => setActivePanel("map")} className="block w-full text-left hover:underline">Map View</button>
          <button onClick={() => setActivePanel("tracks")} className="block w-full text-left hover:underline">My Tracks</button>
          <button onClick={() => setActivePanel("upload")} className="block w-full text-left hover:underline">Upload Track</button>
          <button onClick={() => setActivePanel("account")} className="block w-full text-left hover:underline">My Account</button>
          <button onClick={() => setActivePanel("subscription")} className="block w-full text-left hover:underline text-green-600">Subscription</button>
        ...
        <div className="flex-1 overflow-auto">
          ...
          {activePanel === "tracks" && (
            <div className="p-4 space-y-4">
              ...
              <div className="bg-white shadow rounded-xl p-4">
                <div className="flex justify-between items-center">
                  ...
                  <div className="space-x-2">
                    <button className="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 text-sm">Download</button>
              ...
              <p className="text-gray-400 text-sm italic">Next: Load these from Cloudflare KV</p>
            </div>
          )}
